...
export default function AdminPollResultsScreen() {
  ...
  const { data: results, loading, error, refresh, refreshing, reload } = useFetch(() => pollService.getPollResults(pollId), [pollId]);
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />}>
        {loading ? <LoadingScreen /> : error ? <ErrorView message={getErrorMessage(error)} onRetry={reload} /> : (
          <AppCard style={styles.card}>
            ...
            {results?.options?.length ? results.options.map((o) => (
